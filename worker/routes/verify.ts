...
import { checkPaymentMatchesOrder, findOrderByRazorpayOrderId, markOrderPaid } from "../lib/orders";
...
export async function handleVerify(req: Request, env: WorkerEnv, ctx: ExecutionContext): Promise<Response> {
  ...
  if (!signatureValid) {
    console.error(`verify: bad signature for ${razorpayOrderId}`);
  ...
  let order;
  try {
    order = await findOrderByRazorpayOrderId(env, razorpayOrderId);
  } catch {
    return jsonError(req, 500, "server_error", "Could not verify this payment. Please contact support.");
  }
  if (!order) {
    console.error(`verify: unknown razorpay order ${razorpayOrderId}`);
  ...
  let payment;
  try {
    payment = await fetchRazorpayPayment(env, razorpayPaymentId);
  } catch (error) {
    console.error("verify: payment fetch failed", error);
  ...
  if (!check.ok) {
    console.error(`verify: ${check.code} for order ${order.id} — ${check.detail}`);
  ...
  try {
    ...
    console.error(`verify: fulfilment failed for ${order.id}`, error);
  }
